...
* [`all.sh`](.dotfiles/all.sh): Scripts that work for both zsh and bash
* [`bash`](.dotfiles/bash): Scripts that are for bash only
* [`zsh`](.dotfiles/zsh): Scripts that are for zsh only
...
###.dotfiles/&lt;shell&gt;/*rc
This file contains any rc/profile information for a given shell. [`.dotfiles/all.sh/rc`](.dotfiles/all.sh/rc) contains setup directives that work for both zsh and rsh, and [`.dotfiles/bash/bash4rc`](.dotfiles/bash/bash4rc) contains setup directives specific to bash 4.
...
###[.dotfiles/etc/cache](.dotfiles/etc/cache)
Just a simple cache folder.

###[.dotfiles/etc/man](.dotfiles/etc/man)
Manpages that I've created. As `MANPATH` is updated in [`.dotfiles/all.sh/rc`](.dotfiles/all.sh/rc), these are callable via simply using `man <manpage>`. More information on my manpages can be found in the [**manpages**](#manpages) section below.

###[.dotfiles/etc/setup](.dotfiles/etc/setup)
A folder containing setup for new systems, dotfiles, etc.

###[.dotfiles/etc/templates](.dotfiles/etc/templates)
...
##Both bash and zsh
* [**cpkey**](.dotfiles/all.sh/funcs/cpkey) `cpkey [<keyname>]`: Copies the public key in `~/.ssh/<keyname>.pub` to the clipboard. If `<keyname>` is not specified, it assumes `id_rsa`.
...
* [**mcd**](.dotfiles/all.sh/funcs/mcd) `mcd <dir>`: Creates the directory `<dir>` then changes the current working directory to it.
* [**pman**](.dotfiles/all.sh/funcs/pman):
...
* [**use**](.dotfiles/all.sh/funcs/use) `use <varset>`: Sets a number of variables for use. Currenlty, the available [`<varset>`](.dotfiles/all.sh/funcs/varsets)s are:
	* [`colors`](.dotfiles/all.sh/funcs/varsets/colors): Note that these are only the representation of the 256 color versions of the color. They do not include the leading `\e`, nor the trailing `m`.
* [**venv-helper**](.dotfiles/all.sh/funcs/venv-helper):
...
* [**warn**](.dotfiles/all.sh/funcs/warn) `warn`: Exact same as `echo`, but instead it echoes to `stderr`.
...
##[Both bash and zsh](.dotfiles/all.sh/alias)
...
#Executables
* [**mountramdisk**](.dotfiles/bin/mountramdisk) `mountramdisk`: If the system has over 8 GB of RAM, it mounts a 500MB RAMdisk.
